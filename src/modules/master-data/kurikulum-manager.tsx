...
import { BookOpenCheck, Download, FileUp, Pencil, Plus, RotateCcw, Search, Trash2, X, Save } from "lucide-react";
import Link from "next/link";
import { usePathname, useRouter, useSearchParams } from "next/navigation";

import {
  bulkDeleteKurikulumAction,
  bulkHardDeleteKurikulumAction,
  bulkRestoreKurikulumAction,
  deleteKurikulumAction,
  hardDeleteKurikulumAction,
  importKurikulumAction,
  restoreKurikulumAction,
  saveKurikulumAction,
...
import type { KurikulumRow } from "@/lib/admin/kurikulum";
...
function SubmitButton({ children }: { children: React.ReactNode }) {
  ...
  return <Button type="submit" disabled={pending} className="w-full bg-cyan-600 hover:bg-cyan-700 shadow-lg shadow-cyan-100">{pending ? "Menyimpan..." : <><Save className="mr-2 h-4 w-4" />{children}</>}</Button>;
}
...
function KurikulumFormModal({ open, onClose, item, prodiList }: { open: boolean; onClose: () => void; item: KurikulumRow | null; prodiList: Array<{ id: string; nama: string }>; }) {
  const router = useRouter();
  const [state, formAction] = useActionState(saveKurikulumAction, initialState);

  useActionToast(state, item ? "Data kurikulum diperbarui" : "Kurikulum berhasil ditambahkan");

  useEffect(() => { if (state?.success) { onClose(); router.refresh(); } }, [onClose, router, state?.success]);

  return (
    <ModalShell open={open} onClose={onClose} title={item ? "Edit Kurikulum" : "Tambah Kurikulum"} description="Kelola kurikulum per program studi">
      <form action={formAction} className="space-y-5">
        <input type="hidden" name="id" value={item?.id ?? ""} />
        <div className="grid gap-4 sm:grid-cols-2">
          <div><label className="mb-2 block text-sm font-medium text-slate-800">Kode Kurikulum</label><Input name="kode" defaultValue={item?.kode ?? ""} placeholder="KUR-IF-2026" required /></div>
          <div><label className="mb-2 block text-sm font-medium text-slate-800">Nama Kurikulum</label><Input name="nama" defaultValue={item?.nama ?? ""} placeholder="Kurikulum 2026" required /></div>
        </div>
        <div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div><label className="mb-2 block text-sm font-medium text-slate-800">Tahun Mulai</label><Input name="tahunMulai" type="number" defaultValue={item?.tahun_mulai ?? new Date().getFullYear()} required /></div>
          <div><label className="mb-2 block text-sm font-medium text-slate-800">Total SKS</label><Input name="totalSks" type="number" defaultValue={item?.total_sks ?? 0} required /></div>
          <div><label className="mb-2 block text-sm font-medium text-slate-800">Deskripsi</label><Input name="deskripsi" defaultValue={item?.deskripsi ?? ""} placeholder="Opsional" /></div>
        </div>
        <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
          <input type="checkbox" name="isAktif" id="isAktifKurikulum" defaultChecked={item?.is_active ?? true} className="h-4 w-4 rounded border-slate-300 text-cyan-600" />
          <label htmlFor="isAktifKurikulum" className="text-sm font-medium text-slate-900 cursor-pointer">Kurikulum aktif</label>
        </div>
        <div className="flex justify-end gap-2 border-t border-slate-200 pt-4">
          <Button type="button" variant="secondary" onClick={onClose}>Batal</Button>
          <SubmitButton>{item ? "Simpan Perubahan" : "Tambah Kurikulum"}</SubmitButton>
        </div>
      </form>
    </ModalShell>
  );
...
function DeleteKurikulumModal({ open, onClose, item }: { open: boolean; onClose: () => void; item: KurikulumRow | null; }) {
  const router = useRouter();
  const [state, formAction] = useActionState(deleteKurikulumAction, initialState);

  useActionToast(state, "Kurikulum berhasil dihapus");

  useEffect(() => { if (state?.success) { onClose(); router.refresh(); } }, [onClose, router, state?.success]);
  if (!item) return null;

  return (
    <ModalShell open={open} onClose={onClose} title="Hapus Kurikulum" description="Konfirmasi penghapusan data kurikulum">
      <form action={formAction} className="space-y-5">
        <input type="hidden" name="id" value={item.id} />
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-4">
          <p className="text-sm text-rose-900">Anda yakin ingin menghapus kurikulum berikut?</p>
...
function ImportKurikulumModal({ open, onClose }: { open: boolean; onClose: () => void; }) {
  const router = useRouter();
  const [state, formAction] = useActionState(importKurikulumAction, initialState);

  useActionToast(state, "Import kurikulum selesai");
  ...
  return (
    <ModalShell open={open} onClose={onClose} title="Import Kurikulum" description="Unggah file CSV sesuai template">
      <form action={formAction} className="space-y-5">
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
          Kolom prodi harus sesuai kode program studi yang sudah ada.
        </div>
        <div>
...
export function KurikulumManager({ items, totalItems, totalPages, currentPage, query, prodiList }: { items: KurikulumRow[]; totalItems: number; totalPages: number; currentPage: number; query: string; prodiList: Array<{ id: string; nama: string }>; }) {
  ...
  const [editingItem, setEditingItem] = useState<KurikulumRow | null>(null);
  const [deletingItem, setDeletingItem] = useState<KurikulumRow | null>(null);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [restoreState, restoreAction] = useActionState(restoreKurikulumAction, initialState);
  const [hardDeleteState, hardDeleteAction] = useActionState(hardDeleteKurikulumAction, initialState);
  const [bulkDeleteState, bulkDeleteAction] = useActionState(bulkDeleteKurikulumAction, initialState);
  const [bulkRestoreState, bulkRestoreAction] = useActionState(bulkRestoreKurikulumAction, initialState);
  const [bulkHardDeleteState, bulkHardDeleteAction] = useActionState(bulkHardDeleteKurikulumAction, initialState);

  useActionToast(restoreState, "Kurikulum dipulihkan");
  useActionToast(hardDeleteState, "Kurikulum dihapus permanen");
  useActionToast(bulkDeleteState, "Kurikulum dipindahkan ke sampah");
  useActionToast(bulkRestoreState, "Kurikulum dipulihkan");
  useActionToast(bulkHardDeleteState, "Kurikulum dihapus permanen");
  ...
  return (
    <div className="space-y-6">
      <Card className="overflow-hidden border-white/60 bg-white/80 backdrop-blur-md shadow-xl shadow-slate-200/50">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700"><BookOpenCheck className="h-5 w-5" /></div>
            <div><h3 className="text-xl font-semibold text-slate-900">Data Kurikulum</h3><p className="text-sm text-slate-500">Kurikulum aktif per prodi dan tahun mulai.</p></div>
          </div>
        ...
        <form onSubmit={handleSearchSubmit} className="mt-5 flex flex-col gap-3 md:flex-row">
          <div className="relative flex-1">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cari kode, nama, atau deskripsi kurikulum..." className="pl-10" />
          </div>
        ...
        <div className="mt-5 overflow-x-auto">
          <Table>
            <THead><TR><TH className="w-12 px-3"><Checkbox checked={items.length > 0 && visibleSelectedIds.length === items.length} onCheckedChange={() => toggleAll()} /></TH><TH>Kode</TH><TH>Kurikulum</TH><TH>Prodi</TH><TH>Detail</TH><TH>Status</TH><TH className="w-[160px]">Aksi</TH></TR></THead>
            <TBody>
              {items.length === 0 ? <TR><TD colSpan={7} className="py-10 text-center text-slate-500">Data tidak ditemukan</TD></TR> : items.map((item) => (
                <TR key={item.id}>
                  ...
                  <TD><p className="font-semibold text-slate-900">{item.nama}</p><p className="text-xs text-slate-500 line-clamp-1">{item.deskripsi || "-"}</p></TD>
                  <TD className="text-sm text-slate-600">{item.program_studi?.nama ?? "-"}</TD>
                  <TD className="text-sm text-slate-600">{item.tahun_mulai} | {item.total_sks} SKS</TD>
      ...
      <KurikulumFormModal open={formOpen} onClose={() => setFormOpen(false)} item={editingItem} prodiList={prodiList} />
      <ImportKurikulumModal open={importOpen} onClose={() => setImportOpen(false)} />
      <DeleteKurikulumModal open={Boolean(deletingItem)} onClose={() => setDeletingItem(null)} item={deletingItem} />
    </div>
  );
}
