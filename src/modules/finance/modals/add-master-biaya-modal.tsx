...
import { createMasterBiayaAction } from "@/actions/finance-master";
...
export default function AddMasterBiayaModal({ open, onClose, tahunAkademikList, prodiList }: AddMasterBiayaModalProps) {
  return (
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto rounded-none border-none bg-white shadow-2xl sm:max-w-3xl">
        <DialogHeader className="border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-lg font-black uppercase tracking-tight text-slate-900">Tambah Tarif Kuliah</DialogTitle>
              <DialogDescription className="mt-1 text-xs font-semibold text-slate-500">
                Tarif ini bisa dipakai untuk generate tagihan massal.
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>

        <form action={createMasterBiayaAction} className="grid gap-5 py-2" onSubmit={onClose}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="grid gap-2">
              ...
              <Input id="nama" name="nama" placeholder="SPP Semester Genap" required className="h-11 rounded-none border-2 border-slate-100 bg-slate-50 font-bold" />
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="grid gap-2">
              ...
              <Input id="angkatan" name="angkatan" type="number" min={2000} max={2100} placeholder="2026" className="h-11 rounded-none border-2 border-slate-100 bg-slate-50 font-bold" />
            </div>
          </div>
